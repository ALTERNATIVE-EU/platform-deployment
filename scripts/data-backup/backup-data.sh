...
EOF
  ...
  # Compress data
  kubectl exec -it temp-pod-"${PVC}" -n "${NAMESPACE}" -- apk add tar && kubectl exec -it temp-pod-"${PVC}" -n "${NAMESPACE}" -- tar -czvf /data.tar.gz /data 
  kubectl cp "${NAMESPACE}"/temp-pod-"${PVC}":/data.tar.gz "${LOCAL_DIR}"/"${PVC}".tar.gz
